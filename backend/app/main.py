...
s=get_settings(); app=FastAPI(title='LectureSync API',version='1.0.0')
app.add_middleware(CORSMiddleware,allow_origins=s.cors_origin_list,allow_credentials=True,allow_methods=['*'],allow_headers=['*'])
app.include_router(auth_router,prefix='/api/v1'); app.include_router(lecture_router,prefix='/api/v1')
